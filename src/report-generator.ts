...
export class ReportGenerator {
    ...
    generateMonthlyReport(data: any[], month: string): any {
        ...
        const monthlyData = data.filter(row => 
            this.excelProcessor.isOrderForDate(row, targetDate, false)  // 月報生成時はfalse
        );
    ...
    private calculateRegionStats(data: any[]): any {
        const regions: { [key: string]: { 
            ...
            overtime: number;
            excessive: number;
            excessiveNormal: number; // 69歳以下
            excessiveEarlyElderly: number; // 70-75歳
            excessiveLateElderly: number; // 75歳以上
            single: number;
            singleEarlyElderly: number; // 70-75歳
            singleLateElderly: number; // 75歳以上
    ...
    // 月報・日報用PDF出力（個別ページ処理）
    private async exportMonthlyReportToPDF(pdfContainer: HTMLElement, pdf: any, pageWidth: number, pageHeight: number, marginX: number, marginY: number, messageDiv: HTMLElement): Promise<void> {
        // 各ページを個別に処理
    ...
    private async createPDFHTMLAsync(report: any, type: string): Promise<HTMLElement> {
        const container = document.createElement('div');
        container.className = 'pdf-container';
    ...
    // CSVダウンロード用ヘルパーメソッド
    public async downloadCSV(data: any[][], fileName: string): Promise<void> {
    ...
    async exportYearlyReportsToCSV(data: any[], progressCallback?: (current: number, total: number, month: string) => void, startCallback?: () => void, completeCallback?: (totalMonths: number) => void): Promise<void> {
        try {
     ...
     */
    private calculateEffectiveDate(row: any): Date | null {
        if (!row.date) {
            return null;
        }
    ...
    private createDailyReportPDFHTML(report: any, dateText: string): string {
    ...
    private createRegionCardPDFHTML(regionName: string, stats: any): string {
        if (!stats) {
    ...
    // PDF用年齢統計HTML
    private createAgeStatsPDFHTML(ageStats: any): string {
        if (!ageStats) {
    ...
    // PDF用ランキングテーブルHTML
    private createRankingTablePDFHTML(ranking: any[]): string {
        if (!ranking || ranking.length === 0) {
            return '<div class="text-center text-muted py-3">データがありません</div>';
        }
