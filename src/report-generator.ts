export class ReportGenerator {
    ...
    async exportToPDF(report: any, type: string): Promise<void> {
        try {
            ...
            Object.entries(report.regionStats).forEach(([region, stats]: [string, any]) => {
                if (stats.orders > 0) {
        ...
        try {
            ...
            const csvData = [
                ...
                ...Object.entries(report.regionStats)
                    ...
                    .map(([region, stats]: [string, any]) => [region, `${stats.orders}件 (時間外: ${stats.overtime}件, 過量: ${stats.excessive}件, 単独: ${stats.single}件)`]),
                [''],
                ['年齢別集計', ''],
