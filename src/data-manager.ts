import { HolidaySettings, CalendarSettings } from './types.js';
...
export class DataManager {
    ...
    removePublicHoliday(date: Date): void {
        ...
        this.holidaySettings.publicHolidays = this.holidaySettings.publicHolidays.filter(
            d => this.formatDateForStorage(d) !== dateStr
        );
        this.saveHolidaySettings();
    }
        ...
        try {
            ...
            if (saved) {
                ...
                this.holidaySettings = {
                    publicHolidays: parsed.publicHolidays.map((d: string) => new Date(d)),
                    prohibitedDays: parsed.prohibitedDays.map((d: string) => new Date(d))
                };
            }
        } catch (error) {
    ...
    private readonly STORAGE_KEY = 'daily_monthly_report_data';
    ...
    createDataTableHTML(data: any[], filterState?: { region?: string; age?: string; overtime?: string }): string {
        ...
        const displayColumns = [
            { key: 'date', label: '日付', formatter: (value: any) => this.formatDate(value) },
            { key: 'region', label: '地区', formatter: (value: any) => value },
            { key: 'contractor', label: '契約者', formatter: (value: any) => value },
            { key: 'contractorAge', label: '年齢', formatter: (value: any) => value },
            { key: 'isOvertime', label: '時間外', formatter: (value: any) => value ? '○' : '×' },
            { key: 'isElderly', label: '高齢者', formatter: (value: any) => value ? '○' : '×' },
            { key: 'isExcessive', label: '過量販売', formatter: (value: any) => value ? '○' : '×' },
            { key: 'isSingle', label: '単独契約', formatter: (value: any) => value ? '○' : '×' },
            { key: 'productName', label: '商品名', formatter: (value: any) => value },
            { key: 'amount', label: '金額', formatter: (value: any) => this.formatAmount(value) }
        ];
    ...
    private filterAndSortData(data: any[]): any[] {
        // 日付でソート（新しい順）
    ...
    private createPaginationHTML(totalPages: number, currentPage: number): string {
    ...
    private createFilterStatusHTML(filterState?: { region?: string; age?: string; overtime?: string }): string {
        if (!filterState || (!filterState.region && !filterState.age && !filterState.overtime)) {
            return '';
        }
        
        const activeFilters = [];
        if (filterState.region) activeFilters.push(`地区: ${filterState.region}`);
        if (filterState.age) {
    ...
    // CSVエクスポート機能
    async exportDataToCSV(data: any[]): Promise<void> {
        try {
            // ヘッダー行
            const headers = [
