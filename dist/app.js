...
export class App {
    ...
    // 削除ボタンのクリックイベントハンドラー
    handleHolidayRemove(e) {
        const target = e.target;
        const buttonElement = target.closest('.holiday-remove');
        if (buttonElement) {
    ...
    // ヘッダーの月表示を更新
    updateMonthDisplay(type, year, month) {
    ...
    setupStaffDataSearch() {
        ...
        const elderlySearch = document.getElementById('elderlySearch');
        if (searchExecute) {
    ...
    // 検索条件の判定（件数）
    matchesOrderCondition(actual, condition) {
        if (!condition)
            return true;
        switch (condition) {
    ...
    // 検索結果件数を更新
    updateSearchResultInfo(visibleCount, totalCount) {
    ...
    setupDataFilters() {
        ...
        const filterData = () => {
            ...
            const excessiveFilter = document.getElementById('excessiveFilter');
            ...
            rows.forEach((row) => {
                ...
                if (cells.length >= 15) { // 列数が増えたので15に変更（単独・過量追加）
                    ...
                    if (singleOnly || excessiveOnly) {
                        ...
                        const isExcessive = row.getAttribute('data-excessive') === 'true';
                        if (singleOnly && excessiveOnly) {
        ...
        const clearFilters = () => {
            ...
            const excessiveFilter = document.getElementById('excessiveFilter');
            if (singleFilter)
                singleFilter.checked = false;
            if (excessiveFilter)
                excessiveFilter.checked = false;
            filterData();
    ...
    // データ確認テーブルのCSV出力
    exportDataConfirmationTableToCSV() {
        const table = document.getElementById('dataConfirmationTable');
        if (!table) {
        ...
        try {
            ...
            rows.forEach((row) => {
                ...
                // 表示されている行のみ追加
                if (row.style.display !== 'none' && rowData.length === headers.length) {
    ...
    async exportYearlyCSV() {
        ...
        try {
            ...
            const exportBtn = document.getElementById('exportYearlyCSV');
            ...
            await this.reportGenerator.exportYearlyReportsToCSV(data, (current, total, month) => {
                // プログレス更新
