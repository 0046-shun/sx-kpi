    ...
    isOvertime(date, time, confirmation, confirmationDateTime, age, targetDate) {
        ...
        if (date && time) {
            ...
            if (parsedDate && parsedTime) {
                ...
                const contractorTime = new Date(`${dateStr}T${parsedTime}:00`);
                if (!isNaN(contractorTime.getTime())) {
                    const cutoffTime = new Date(contractorTime);
                    cutoffTime.setHours(18, 30, 0, 0);
                    if (contractorTime >= cutoffTime) {
