export class BITool {
        ...
        try {
            ...
            if (settings) {
                ...
                const uniquePublicHolidays = settings.publicHolidays ?
                    Array.from(new Set(settings.publicHolidays.map((d) => d.toISOString().split('T')[0])))
                        .map(dateStr => new Date(dateStr)) : [];
                const uniqueProhibitedDays = settings.prohibitedDays ?
                    Array.from(new Set(settings.prohibitedDays.map((d) => d.toISOString().split('T')[0])))
                        .map(dateStr => new Date(dateStr)) : [];
                return {
    ...
    getDayType(date) {
        ...
        if (holidaySettings) {
            ...
            if (holidaySettings.publicHolidays && Array.isArray(holidaySettings.publicHolidays)) {
                const isHoliday = holidaySettings.publicHolidays.some((holidayDate) => {
            ...
            if (holidaySettings.prohibitedDays && Array.isArray(holidaySettings.prohibitedDays)) {
                const isProhibited = holidaySettings.prohibitedDays.some((prohibitedDate) => {
    ...
    // BIダッシュボードのHTMLを生成
    createBIDashboard(data, targetMonth) {
    ...
    // 統計データを計算
    calculateStats(data, targetMonth) {
        const stats = {
            totalOrders: 0,
            totalAmount: 0,
            averageAmount: 0,
        ...
        const staffAmount = new Map();
        const regionCount = new Map();
        const regionAmount = new Map();
    ...
    renderRegionOrdersChart(stats) {
        const ctx = document.getElementById('regionOrdersChart');
        if (!ctx)
            return;
    ...
    renderRegionAmountsChart(stats) {
        const ctx = document.getElementById('regionAmountsChart');
        if (!ctx)
            return;
...
//# sourceMappingURL=bi-tool.js.map