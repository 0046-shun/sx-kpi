export class ReportGenerator {
    ...
    generateDailyReport(data, date) {
        ...
        if (this.calendarManager) {
            ...
            console.log('公休日設定:', settings.publicHolidays.map((d) => d.toLocaleDateString()));
            console.log('禁止日設定:', settings.prohibitedDays.map((d) => d.toLocaleDateString()));
        }
        else {
        ...
        data.forEach(row => {
            ...
            if (row.staffName && row.regionNumber && row.departmentNumber) {
                ...
                if (existingStaff) {
                    ...
                    if (row.contractorAge && row.contractorAge >= 70)
